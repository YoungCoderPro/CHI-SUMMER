/* ============================================================
   ...
   • Leaflet map, pins color-coded BY CATEGORY
...
/* ---------- map ---------- */
function initMap() {
  const map = L.map("map", { scrollWheelZoom: false, zoomControl: true }).setView(CHICAGO_CENTER, 12);
  state.map = map;
...
function drawStrides(map) {
  fetch("data/streets.geojson").then((r) => r.json()).then((geo) => {
    L.geoJSON(geo, { style: { color: "#6B4EE6", weight: 3, opacity: 0.75, lineCap: "round" } }).addTo(map);
  }).catch(() => {});
}

function drawHome(map) {
  const icon = L.divIcon({ className: "", html: `<div class="pin-home" title="Home base"></div>`,
    iconSize: [24, 24], iconAnchor: [12, 12] });
  L.marker([HOME.lat, HOME.lng], { icon, zIndexOffset: 1000 }).addTo(map)
    .bindTooltip(`<div class="tip-meta"><div class="tip-name">${esc(HOME.name)}</div><div class="tip-hood">🏠 Home base · West Loop</div></div>`,
      { className: "place-tip", direction: "top", offset: [0, -14] });
}

function pinIcon(p) {
  const color = catColor(p.type);
  const want = p.status !== "visited";
  const glyph = p.favorite ? `<span class="glyph">${starSVG(11, "#fff")}</span>`
              : p.status === "visited" ? `<span class="glyph">✓</span>` : "";
  const html = `<div class="pin ${want ? "is-want" : ""}" style="background:${color}">${glyph}</div>`;
  return L.divIcon({ className: "", html, iconSize: [20, 20], iconAnchor: [10, 20], popupAnchor: [0, -18] });
}
...
function drawPlaces() {
  ...
  visiblePlaces().forEach((p) => {
    ...
    const marker = L.marker([p.lat, p.lng], { icon: pinIcon(p) })
      .addTo(state.map)
      .bindTooltip(tooltipHTML(p), { className: "place-tip", direction: "top", offset: [0, -16], opacity: 1 })
      .on("click", () => openPanel(p.id));
...
function submitAdd(e) {
  ...
  if (Number.isFinite(lat) && Number.isFinite(lng)) state.map.flyTo([lat, lng], 14, { duration: 0.8 });
